...
export async function clickConversation(conversationId: string) {
  ...
  mutate(
    ...
    (currentData: any) => {
      ...
      const conversations: IChatConversations = currentData !== undefined && Array.isArray(currentData.conversations) === true && currentData.conversations.map(
        (conversation: IChatConversation) =>
